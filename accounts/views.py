...
def register_staff(request):#sinartisi pou servirei tin kentriki selida tou site
    if request.method=="POST": 
        #I take the data from POST
        username=request.POST["username"]
        password=request.POST["password"]
        ...
        if User.objects.filter(username=username).exists():#iparxei idi to username
            ...
            user=User.objects.create_user(username=username,password=password,email=email,first_name=first_name,last_name=last_name,is_staff=1)   
            user.save()
            messages.success(request,f"User created succesfully with username : {username}")
            return redirect("/accounts/registerstaff")
